...
public class StriderEntityRenderer extends EntityRenderer {
     ...
     */
    private void orientCamera(float dTick)
    {
        Entity entity = this.mc.getRenderViewEntity();
        float f1 = entity.getEyeHeight();
        double d0 = entity.prevPosX + (entity.posX - entity.prevPosX) * (double)dTick;
        double d1 = entity.prevPosY + (entity.posY - entity.prevPosY) * (double)dTick + (double)f1;
        double d2 = entity.prevPosZ + (entity.posZ - entity.prevPosZ) * (double)dTick;

        float yaw = this.prevCameraYaw + (this.cameraYaw - this.prevCameraYaw) * dTick + 180.0F;
        float pitch = this.prevCameraPitch + (this.cameraPitch - this.prevCameraPitch) * dTick;
        float roll = 0.0F;
        
        Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, dTick);
        net.minecraftforge.client.event.EntityViewRenderEvent.CameraSetup event = new net.minecraftforge.client.event.EntityViewRenderEvent.CameraSetup(this, entity, block, dTick, yaw, pitch, roll);
        net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(event);

        d0 = entity.prevPosX + (entity.posX - entity.prevPosX) * (double)dTick;
        d1 = entity.prevPosY + (entity.posY - entity.prevPosY) * (double)dTick + (double)f1;
        d2 = entity.prevPosZ + (entity.posZ - entity.prevPosZ) * (double)dTick;
        this.cloudFog = this.mc.renderGlobal.hasCloudFog(d0, d1, d2, dTick);
    }
...
//            this.mc.thePlayer.setAngles(f3, f4 * (float)b0);
    ...
    private void setCameraAngles(float yaw, float pitch) {
        ...
        float f3 = this.cameraYaw;
        this.cameraYaw = (float)((double)this.cameraYaw + (double)yaw * 0.15D);
        this.cameraPitch = (float)((double)this.cameraPitch - (double)pitch * 0.15D);
        this.cameraPitch = MathHelper.clamp_float(this.cameraPitch, -90.0F, 90.0F);
        this.prevCameraPitch += this.cameraPitch - f2;
        this.prevCameraYaw += this.cameraYaw - f3;
	}
